...
global {
	...
	init {
		...
		//		} 
		file patient_csv_file1 <- csv_file("0904687610_filter.csv", true);
		file patient_csv_file2 <- csv_file("0964395113_filter.csv", true);
...
species Patient {
	...
	//		if ((this_date plus_days date_idx) >= created) {
		...
		draw circle(100) at: location color: #red;
		//		}
...
species adm3 {
	...
	aspect default {
		if (#zoom > 7) {
			draw VARNAME_3 at: location perspective: false;
		}
...
experiment generateGISdata type: gui {
	float minimum_cycle_duration <- 0.055;
	...
	output {
		display map type: opengl draw_env: false background: background {
			...
			image file: "VNM.27_1satellite.png" refresh: false;
